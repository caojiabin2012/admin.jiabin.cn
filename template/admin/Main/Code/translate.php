...
function jiabin(type)
{
      $.post("<?=Utils::makeUrl('Main/Code', 'translateData')?>",{str:$('#baseText').val(), type:type},function(data){
        $('#newText').text(data.pinyin);
      }, 'json');
}
        ...
        <div class="panel-heading">
            <div class="media">
                <div class="media-body">
                    <h4 class="media-heading">在线文本转换拼音工具</h4>
                    <div>在线文本转换拼音工具</div>
                </div>
            </div>
        </div>
            ...
            <form class="form-inline" role="form" method="post" target="_blank" action="/code/compare.html">
            	<div class="row">
					...
					<div class="col-md-6">
		                <div class="form-group">
		                    <div class="input-group">
								<textarea class="form-control" id="newText" rows="15" cols="100" placeholder="转换完的内容"></textarea>
		                    </div>
		                </div>
					</div>
				</div>
				...
				<br/>
				<p class="text-left">
	                <button type="button" onclick="jiabin(0)" class="btn btn-primary">内容翻译</button>
	                <button type="button" onclick="jiabin(1)" class="btn btn-primary">姓名翻译</button>
